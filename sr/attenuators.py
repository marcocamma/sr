...
class Filters:
    def __init__(self, att_list, get_E=None):
        ...
        self._has_status = all(
            [all([f.is_in is not None for f in a]) for a in att_list]
        )
        self._att = list(
        ...
        for att in self._att:
            attenuators = [
                self._att_list[axis][filtnum] for axis, filtnum in enumerate(att)
            ]
            ...
            if axis != 0 and all(
                [
                    not isinstance(attenuators[i], EmptySlot)
                    for i in range(last_non_empty_axis)
                ]
            ):
                temp.append(att)
    ...
    def calc_transmission(self, E=None, filter_selection=None):
        ...
        if E is None:
            E = self.get_E()
        if E is None:
            raise ValueError("Energy not provided or not able to get it")

        if filter_selection is None:
            filter_selection = self._status()
        if filter_selection is None:
            raise ValueError("Filter selection not provided or not able to get it")
    ...
    def _calc_all_transmissions(self, E):
        if E not in self._CACHE:
            data = np.asarray(
                [self.calc_transmission(E, filters) for filters in self._att]
            )
            self._CACHE[E] = data
    ...
    def _calc_all_transmissions_protect_downbeam(self, E):
        if E not in self._CACHE_protect_downbeam:
            data = np.asarray(
                [
                    self.calc_transmission(E, filters)
                    for filters in self._att_protect_downbeam
                ]
            )
    ...
    def calc_best_transmission(
        self,
        requested_transmission,
        E=None,
        verbose=False,
        use_protect_downbeam=False,
        move=False,
        wait_move=0.1,
    ):
        """E must be a float, can't be a vector"""
        ...
        if move:
            for iaxis, chosen_filter in enumerate(best_combination):
                ...
                if f.move_in is None:
                    raise ValueError("No moving function defined!")
                f.move_in()
